...
<?php
//fbrcode to Get Sales Type and Payment Type from POS - 
if ($payment['payment_type'] == 'Cash')
{ $pt = 1;}
elseif($payment['payment_type'] == 'Debit Card' || $payment['payment_type'] == 'Credit Card')
{ $pt=2;}
elseif($payment['payment_type'] == 'Mixed')
{$pt=5;}
...
foreach($cart as $line=>$item)
	{
$dataitem=array ('ItemCode'=>$item['item_number'],
'ItemName'=>$item['name'],
...
'TaxCharged'=>$tax['sale_tax_amount'],
'InvoiceType'=>$it,
'RefUSIN'=>null);
array_push($iitems, $dataitem);
}

//print_r($iitems);

/*
This section connects to FBR Fisicial Module and gets the FBR Invoice Number
*/

$url = 'http://localhost:8524/api/IMSFiscal/GetInvoiceNumberByModel'; // Local URL for FBR Fisical Module
$dt=date("Y-m-d H:i:s");
$datapost = array('InvoiceNumber'=>$sale_id,
'POSID'=>XXXXX, // Your FBR POSID
'USIN'=>'USIN0',
'DateTime'=>$dt,
'TotalQuantity'=>$item['quantity'],
'TotalBillAmount'=>$total,
'TotalSaleValue'=>$subtotal,
'TotalTaxCharged'=>$tax['sale_tax_amount'],
'PaymentMode'=>$pt,
'RefUSIN'=>null,
'InvoiceType'=>$it,
'Items'=>$iitems);
echo $data_string = http_build_query($datapost);
echo '----------------------------';
//open connection
$ch = curl_init();
...
$bc = $obj->InvoiceNumber; // Invoice Number as returned by FBR Fisical Module
$bbc =$this->barcode_lib->generate_receipt_barcode($bc); #Generating Barcode based on FBR Invoice Number to be printed on the receipt
?>
<center>
<div id="barcode"><strong>This Receipt / Invoice is verified by FBR POS Invoicing System</strong><br><br>
		<img src='data:image/png;base64,<?php echo $bbc ?>' /><br>
		</div>

/*
For Ease of Use Updating Sales table with FBR Invoice Number received for future Use.
*/
<?php
...
$sql = "UPDATE ospos_sales SET fbr_no= '$bc' WHERE sale_id='$sale_id'"; // Updateing the sales tables with FBR Invoice Number

$conn->query($sql);
$conn->close();
echo $obj->InvoiceNumber;?>
<br>
<?php
echo $obj->Response;
?>
